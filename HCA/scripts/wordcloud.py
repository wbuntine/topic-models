...
# 
# THIS VERSION modified by Wray Buntine <wray.buntine@monash.edu> 
# to work with hca scripts.
#
# License: MIT
...
class WordCloud(object):
    ...
    def generate_from_frequencies(self, frequencies):
        ...
        # for word, freq, rank in frequencies:
        #    print("I: ", word, freq, rank)

        frequencies = [ (word, (float(freq)/max_frequency), float(rank)) for word, freq, rank in frequencies ]
    ...
    def process_text_data(self, text):
        """Text input is triples (word,freq,rank) in text
           formatted as 'W1,F1,R1 W2,F2,R2'
        ...
        Returns
        -------
        words : list of tuples (string, float, float)
            Word tokens with associated frequency, rank.
        """
        words = text.split()
        freqs = []
        for word in words:
    ...
    def generate_from_text(self, text):
        ...
        self
        """
        words = self.process_text_data(text)
    ...
    def to_image(self):
        ...
        for (word, count, rank), font_size, position, orientation, color in self.layout_:
