...
foreach my $A ( @tag ) {
  &mysystem("$VERSION $Q $X $Y $FILE -t$TRAIN  -Llike,0,0 -v $command{$A} -C$GIBBSTRAIN $STEM ${TRIAL}$A", "hca train");
}
#   do doc-topic probability estimation over $GIBBSEVAL samples
#   and switch off likelihood calcs at end
foreach my $A ( @tag ) {
  &mysystem("$VERSION $Y $Q $FILE -C$GIBBSEVAL $BUILDPROB -Llike,0,0 -lphi,1,2 $DIAGNOSTICS -v -v -r 0 -T$TEST $STEM ${TRIAL}$A","hca eval");
}
if ( $doclass ) {
   ...
   foreach my $A ( @tag ) {
     &mysystem("$VERSION $Y $FILE $PMI -C0 -X -v $HOLD -Llike,$GIBBSEVAL,$GIBBSEVALBURN -v -V -r 0 -T$TEST $STEM ${TRIAL}$A","hca test");
     if ( $USESVM ) {
	if ( "${TRIAL}$A.testprob" ) {
   ...
   foreach my $A ( @tag ) {
     &mysystem("$VERSION $Y $FILE $PMI -C0 -v $HOLD -Llike,$GIBBSEVAL,$GIBBSEVALBURN -v -V -r 0 -T$TEST $STEM ${TRIAL}$A","hca test");
   }
}
if ( $usephi ) {
   foreach my $A ( @tag ) {
...
Options:
    ...
    -Y ARGS             args given to every run
