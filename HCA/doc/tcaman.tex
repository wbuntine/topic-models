...
\item[mu]
Load up the \texttt{mu} matrix from file \File{RepStem.mu}
...
\item[phi]
Load up the \texttt{phi} matrix from files \File{RepStem.phiNNN}
...
Stored in \File{RepStem.mu}.
\item[phi] 
Estimate the word probability vector for each topic.
Stored in the \File{RepStem.phiNNN} files where
...
with whats in the \File{DataStem.epoch} file.
\item[\Opt{-V}]  load the dictionary from the
\File{DataStem.tokens} file for use in reporting.  It has one token per line.
\end{Description}
...
\end{Description}
Note the \File{RepStem.phiNNN} and \File{RepStem.mu} files are also used
as input with the \OptArg{-r}{phi} and \OptArg{-r}{mu} options
respectively.
...
(the actual data counts are totaled from \File{RepStem.cdt}).
...
corresponding to \File{RepStem.mevt}.
