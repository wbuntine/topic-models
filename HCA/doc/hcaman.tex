...
that does various versions of non-parametric topic models using Gibbs sampling including LDA, HDP-LDA, and NP-LDA all with/without burstiness modelling.  
Another variant uses a normalised Gamma distribution on topics
with/without an Indian buffet process (IBP) to switch off topics in documents.
Various diagnostics, ``document completion'' testing and coherence measurements with PMI are also supported. The code
...
\item[alpha:] this is the prior on topic vector (\texttt{theta}) for each document.
LDA has a simple symmetric Dirichlet with parameter alpha
and the vector has dimension $T$ (the number of topics).
\item[beta:] this is the prior on word vector  (\texttt{phi}) for each topic.
...
The vector \texttt{theta} and its various priors and parameters is the Alpha side
and the vector \texttt{phi} and its various priors and parameters is the Beta side.
...
\item[{-A}{value[,file]}]  Use a symmetric Dirichlet prior on \texttt{theta}
...
Dirichlet on \texttt{theta} to a Pitman-Yor process, thus
allowing estimation of hierarchical prior.
It defines a distribution on \texttt{theta} and its prior mean (a vector) 
 \texttt{alpha} of \texttt{type} as follows:
\begin{Description}[hpdd]
\item[hdp] 
\texttt{theta} is modelled with a Dirichlet Process 
...
\item[hpdd] \texttt{theta} is modelled with a Pitman-Yor Process
...
\item[pdp] \texttt{theta} is modelled with a Pitman-Yor Process
with mean \texttt{alpha}, discount \texttt{a} and concentration \texttt{b},
...
\end{Description}
\item[{-A}{ng}]
  The instigates the \emph{normalised Gamma} prior on \texttt{theta}.
...
parameters now, the vector of alpha parameters for each topic,
\texttt{NGalpha}, and the
vector of beta parameters for each topic, \texttt{NGbeta}.
...
Also, introduce Indian Buffet Process (IBP) priors on the topics
by setting the \texttt{ngs0} parameter or the
\texttt{ngs1} parameter,
for instance, ``\OptArg{-S}{ngs0=1000} \OptArg{-S}{ngs0=10}''
means a Beta(\texttt{ngs0,ngs1}) prior is used for the probability
of a topic not being sparse.
That is, a higher \texttt{ngs0} increases sparsity.
By default the IBP priors are inactive.
...
\begin{Description}[hpdd]
\item[hdp] \texttt{phi} is modelled with a Dirichlet Process
...
\item[hpdd] 
\texttt{phi} is modelled with a Pitman-Yor Process
...
\item[a] discount parameter for the non-parametric distribution
  on the \texttt{theta}, topic distribution per document.
\item[b] concentration parameter for the non-parametric distribution
  on \texttt{theta}, the topic distribution per document.
\item[a0] discount parameter for the non-parametric distribution
  on alpha, the prior for \texttt{theta}.
...
\item[aw] discount parameter for the non-parametric distribution
  on \texttt{phi}, word distribution per topic.
...
\item[alpha] same as \OptArg{-A}{value}.
\item[beta] same as \OptArg{-B}{value}.
\item[ngs0] switch on IBP setting of topics in documents,
  and set the Beta prior to be Beta(\texttt{ngs0},1).
Only works when \OptArg{-A}{ng} is in use.
\item[ngs1] modify the second argument of the
     Beta prior to get Beta(\texttt{ngs0},\texttt{ngs1}).
\end{Description}
\end{Description}
...
The vector hyperparameters \texttt{bdk}
...
word is held out in estimating the latent variables (like \texttt{theta})
...
\item[sparse] 
Estimate topic sparsity in the \texttt{theta} matrix for the
words given in \File{DataStem.smap}.
If \File{DataStem.smap} is not there then this defaults to all words.
Note, the default can be quite wasteful for multicore, is it duplicates the \texttt{theta} matrix
...
\item[phi:] rank by computed \texttt{phi} value (if loaded).
...
a pre-estimated \texttt{phi} matrix (for efficiency).
\item[\OptArg{-R}{topk}] 
Do a prediction of the next \texttt{topk} items to be included in the document,
for each document in the training set.  Must have \texttt{Phi} and
\texttt{Theta} loaded.
Output to \File{DataStem.topk}.
...
integer df for the corresponding word, matching \File{DataStem.tokens}.
...
Otherwise, if the \texttt{phi} matrix is being estimated 
...
If the \OptArg{-A}{ng} option is used then vectors
\texttt{NGalpha} and \texttt{NGbeta} are saved as well,
as well as settings for the Beta prior for the IBP,
as \texttt{ngs0} and \texttt{ngs1}.
...
\item[{-Adir} {-Bdir} :] this is standard LDA using the default settings
  ...
  initialised.   
\item[{-B0.001} :] this is HDP-LDA using a Dirichlet prior for \texttt{phi}   (word probability vector),
  and a default non-parametric prior (HPDD) for
  \texttt{theta} (topic probability vector).
\item[\texttt{default}:]
  default is full non-parametric topic modelling \emph{without} burstiness,
  we call NP-LDA,
  both priors for \texttt{theta} and \texttt{phi} use the
  default non-parametric prior (HPDD).
\item[{-Ang} :]
  this uses a normalised Gamma for the
  prior for \texttt{theta}, which means different dimensions
...
If IBP sparsity modelling is instigated with \OptArg{S}{ngs0},
then a third probability gives the
proportion of this topic that is sparse.
...
which does not work with plain Dirichlet priors on \texttt{phi}:
\begin{verbatim}
Topic 5/0 p=13.68% ws=37.0% ds=43.3% ew=732 ewp=424.9 ed=132.3 ...
...
The burstiness version significantly improves perplexity and
often times coherence of topics.  Though it is not tested with all options.
...
are done to train a \texttt{theta} for the test document and the remaining 1/4
...
200 cycles and \texttt{theta}, \texttt{phi} and alpha are estimated at that stage.
Finally, a full diagnostic report is done
(without holdout testing, though) to report on
the words in the topics using the estimated \texttt{phi}.
\begin{verbatim}
...
The third example uses the normalised gamma model for topics along with
sparsity using a Beta(\texttt{1000},\texttt{20}) prior.
The second command prints a report.
\begin{verbatim}
hca -K$K -C500 -q4 -v -Ang -Sngs0=1000 -Sngs1=20 $DATA $STEM/nng
hca -r0 -C0 -v -v -V -e $DATA $STEM/nng
\end{verbatim}
The report will give topic details, which includes the usual diagnostics line
per topic discussed in the Diagnostics Report section,
which towards the end has an entry like
``ng=0.0754,1.7225,0.4785.''
This says the mean probability for this topic is
0.0755, the variance multiplier (increase over a standard
Dirichlet's variance) is 1.72,
and topics are sparse in documents with probability 0.4785.
The higher the variance multiplier, the more specialised
the topic is (i.e., it tends to be higher sometimes and other
times lower than average).
The actual \texttt{alpha} and \texttt{beta}
vectors for the Gamma distributions
are printed in the file \File{RepStem.par}.
