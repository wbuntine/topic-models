...
\item[\OptArg{-A}{value}]  Use a symmetric Dirichlet prior on theta
using this \texttt{value} for each dimension.  The value must be a positive float.
...
and beta is modelled with a (truncated) GEM 
...
{\tt b}, {\tt b0}, {\tt betatot}, with
{\tt b} and {\tt b0} being sampled every 3 major cycles and {\tt betatot}
every 4 major cycles;
\item
in this case {\tt a} and {\tt a0} are not sampled because they are fixed at 0,
...
\item
{\tt PYbeta=0} means the beta side is a Dirichlet;
\item
{\tt PYalpha=2}  means the alpha side is a truncated GEM prior at the top
level and Pitman-Yor process or Dirichlet process at the document level;
\item
and {\tt TEST=0} means there is no test data.
\end{itemize}
...
After {\tt Pars:} appears the list of hyperparameters being sampled and their
current values.  Note the parameter {\tt bdk}, the
concentration for the bursiness per topic, is
a vector over topics, so only the first entry is printed.

Adding an extra level of verbosity using an addition \Opt{-v}, one gets
...
to sample {\tt b}.  The initial value was 3.432078
...
If some topics have no data in them, {\tt empty} will tell how much.
The effective number of topics is {\tt 19.049888},
